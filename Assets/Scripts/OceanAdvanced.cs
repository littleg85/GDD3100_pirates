...
public class OceanAdvanced : MonoBehaviour
{
  ...
  public void RegisterInteraction(Vector3 pos, float strength)
  {
    ...
    interactions[interaction_id].w = Time.timeSinceLevelLoad;
    ocean.SetVectorArray("interactions", interactions);
  ...
  static public float GetWaterHeight(Vector3 p)
  {
    float height = 0;
    for (int i = 0; i < NB_WAVE; i++)
      height += waves[i].amplitude * Mathf.Sin(Vector2.Dot(waves[i].direction, new Vector2(p.x, p.z)) * waves[i].frequency + Time.timeSinceLevelLoad * waves[i].phase);
    return height;
  }
}
